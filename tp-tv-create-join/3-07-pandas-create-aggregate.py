...
print(animals.dtype)
print("On constate que les éléments du tableau sont de type unicode codé sur 32 bits")
...
print(df.loc[2]["lifespan"], type(df.loc[2]["lifespan"]))
print("On constate que la valeur de lifespan est du type chaîne de caractère alors que l'on souhaiterait plutôt un flottant.")

# %%
## 5

print(df.loc[2]["names"], type(df.loc[2]["names"]))
...
# %% [markdown]
# On n'obtient pas un array numpy mais une méthode.

# %%
## 9

df[["speed", "lifespan"]] = df[["speed", "lifespan"]].astype(np.float32)
...
dfi = df1i.merge(df2i, left_index = True, right_index = True)
dfi = dfi.append(df3i)
df
