...
mask = is_empty_column(df, df.columns)
...
print(type(unique1))
print(unique1 == np.array([1., 2., 3., np.nan]))
np.all(unique1 == np.array([1., 2., 3., np.nan]))
```

Cela ne renvoie pas `True` car le nan de numpy n'est pas le nan pandas.
...
lines_to_clean = np.arange(0, 8403)[df["cLT2FREQ"].isna()]
df.drop(index = lines_to_clean, inplace = True)
df.head(10)
```
